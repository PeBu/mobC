...
// http://stackoverflow.com/questions/16095155/javascript-contenteditable-set-cursor-caret-to-index 
 // http://www.codescratcher.com/javascript/get-or-set-cursor-position-in-a-textarea/
 // http://angular-js.in/cell-cursor/
...
var VKI = function(customConfig, layout, deadKeys, keyInputCallback) {
  ...
  this.VKI_enterSubmit = config.enterSubmit || true; // true to Submit forms when Enter is pressed. Fn to execute a custom function.
...
function hideKeyboard(element) {
  ...
  // new -> P.B.
  self.resetFocus = function (elem) {
    var count = self.VKI_target.form.elements.length;

    for (var z = 0; z < count; z++) {
      
      if (elem && elem !== self.VKI_target.form.elements[z]) {
        self.VKI_target.form.elements[z].removeAttribute('_hasFocus')
        self.VKI_target.form.elements[z].className = ''

        if (!self.VKI_target.form.elements[z].getAttribute("_wasReadonly")) {
          //self.VKI_target.form.elements[z].removeAttribute('readonly')
        }
  ...
  self.VKI_targetSetFocus = function() {
    //self.VKI_target.focus()
  }
  
  // new -> P.B. 
  self.setFocus = function (elem) {
  ...
  // mod P.B. Attach the keyboard to an element
  self.attachVki = function (elem) {

    // check if allready attached
    if (elem.getAttribute("VKI_attached")) {
      return false;
    };

    // add atached attribute
    elem.setAttribute("VKI_attached", 'true');

    // if element is "real" readonly 
    if (typeof elem.getAttribute('readonly') === 'string') {
      elem.setAttribute('_wasReadonly', true);
    }
    ...
    elem.onfocus = function (e) {
      console.log('got focus', e)
      
      if (!elem.getAttribute("_wasReadonly")) {
        elem.className = '_hasFocus'  
      }
      ...
      // we have the focus allready or real readonly -> nothing to do
      if (elem.getAttribute("_hasFocus") || elem.getAttribute("_wasReadonly")) {
        return false;
      };
      ...
      if (self.VKI_target !== this) {
        ...
        self.resetFocus(elem)
        elem.setAttribute('_hasfocus', true);
        //elem.className = '_hasFocus'
      }
    ...
    angular.element(elem).bind('VKI_close', function () {
      self.VKI_close(false);
    });
  };
...
*/
                  ...
                  case "Tab":
                    VKI_addListener(td, 'click', function() {
                      if (self.VKI_activeTab) {
                        ...
                        } else self.VKI_targetSetFocus(); //self.VKI_target.focus();
                      ...
                      if (self.VKI_target.nodeName != "TEXTAREA") {
                        ...
                        if (typeof self.VKI_enterSubmit === 'function') {
                          ...
                          for (var z = 0; z < count; z++) {
                            ...
                            // if its me
                            if (self.VKI_target === self.VKI_target.form.elements[z] ) {
                              ...
                              // find next (not readonly) field 
                              for (var k = z+1; k < count; k++) {
                                
                                if (!self.VKI_target.form.elements[k].getAttribute("_wasReadonly")) {
                                  nextField = self.VKI_target.form.elements[k];
                                  break;
                                };
                              ...
                              // set focus
                              if (nextField) {
                                //self.setFocus(nextField);
                                
                                nextField.focus();
                                //angular.element(nextField)[0].focus();
                              }
                              else {
                                self.VKI_close(false);
                              }
  ...
  this.VKI_modify = function(type) {
    ...
    for (var x = 0; x < tables.length; x++) {
      ...
      for (var y = 0; y < tds.length; y++) {
        ...
        var className = [];
        var lkey = this.VKI_layout[this.VKI_kt].keys[x][y];
  ...
  this.VKI_insert = function(text) {
    ...
    placeCaretAtEnd(this.VKI_target)
    ...
    console.log('ml',this.VKI_target.maxlength)
    console.log('vl',this.VKI_target.value.length, this.VKI_target.createTextRange)
